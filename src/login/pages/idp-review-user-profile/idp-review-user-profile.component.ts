...
@Component({
  ...
  providers: [
    ...
    { provide: ComponentReference, useExisting: forwardRef(() => IdpReviewUserProfileComponent) },
  ],
})
export class IdpReviewUserProfileComponent extends ComponentReference {
  ...
  registrationSteps = computed(() => {
    ...
    // 仅保留 idp-review-user-profile 实际显示的字段
    const reviewFields = fields.filter(
      (f) => f === 'username' || f === 'email' || f === 'firstName' || f === 'lastName',
    );

    const steps: string[][] = [];
    for (let i = 0; i < reviewFields.length; i += 2) {
      steps.push(reviewFields.slice(i, i + 2));
    }
    return steps;
  });
